...
class SSN2D(BaseLocalizer):
    ...
    def forward_train(self,
        ...
        num_per_video = img_group.shape[1]
        num_seg = img_group.shape[2]
        img_group = img_group.reshape((-1, self.in_channels) + img_group.shape[4:])
        ...
        if self.dropout is not None:
            x = self.dropout(x)
        activity_feat, completeness_feat = self.segmental_consensus(x, prop_scaling)
        losses = dict()
        if self.with_cls_head:
            activity_score, completeness_score, bbox_pred = self.cls_head((activity_feat, completeness_feat))
            loss_cls = self.cls_head.loss(activity_score, completeness_score, bbox_pred,
                                          prop_type, prop_labels, reg_targets, self.train_cfg)
    ...
    def forward_test(self,
        ...
        for ind in range(0, num_ticks, minibatch_size):
            chunk = img_group[:, ind:ind + minibatch_size, ...].view((-1,) + img_group.shape[2:])
        ...
        if not self.is_test_prepared:
            self.is_test_prepared = self.cls_head.prepare_test_fc(self.segmental_consensus.feat_multiplier)
        ...
        activity_scores, completeness_scores, bbox_preds = self.segmental_consensus(output, prop_tick_list, scaling_list)
        if bbox_preds is not None:
            bbox_preds = bbox_preds.view(-1, self.cls_head.num_classes, 2)
            bbox_preds[:, :, 0] = bbox_preds[:, :, 0] * reg_stats[1, 0] + reg_stats[0, 0]
            bbox_preds[:, :, 1] = bbox_preds[:, :, 1] * reg_stats[1, 1] + reg_stats[0, 1]


        return rel_prop_list.cpu().numpy(), activity_scores.cpu().numpy(), completeness_scores.cpu().numpy(), bbox_preds.cpu().numpy()
