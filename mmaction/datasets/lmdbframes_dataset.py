...
class LMDBFramesDataset(Dataset):
    ...
    def _get_frames(self, record, key_tmpl, modality, indices, skip_offsets):
        ...
        self.env = lmdb.open(osp.join(self.lmdb_prefix, self.lmdb_tmpl.format(record.path)),
                             max_readers=1,
                             readonly=True,
                             lock=False,
                             readahead=False,
        ...
        for seg_ind in indices:
            ...
            for i, ind in enumerate(range(0, self.old_length, self.new_step)):
                seg_imgs = self._load_image(key_tmpl, modality, p + skip_offsets[i]) 
    ...
    def __getitem__(self, idx):
        ...
        img_group = self._get_frames(record, key_tmpl, modality, segment_indices, skip_offsets)
        ...
        for i, (modality, key_tmpl) in enumerate(zip(self.modalities[1:], self.key_tmpls[1:])):
            img_group = self._get_frames(record, key_tmpl, modality, segment_indices, skip_offsets)
