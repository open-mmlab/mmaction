...
from .utils import (to_tensor, random_scale, parse_directory,
...
class SSNVideoRecord(object):
    def __init__(self, prop_record):
        ...
        self.gt = [
            SSNInstance(int(x[1]), int(x[2]), frame_count, label=int(x[0]), best_iou=1.0) for x in self._data[2] if int(x[2]) > int(x[1])
        ]
...
class SSNDataset(Dataset):
    def __init__(self,
        ...
        if 'normalized_' in ann_file:
            ...
            if not osp.exists(self.proposal_file):
                print('{} does not exist. Converting from {}'.format(self.proposal_file, ann_file))
                frame_dict = parse_directory(self.img_prefix, key_func=lambda x: x.split('/')[-1])
                process_localize_proposal_list(ann_file, self.proposal_file, frame_dict)
        ...
        print("{} out of {} videos are valid.".format(len(valid_inds), len(self.video_infos)))
        self.video_infos = [self.video_infos[i] for i in valid_inds]

        self.video_dict = {record.video_id: record for record in self.video_infos}
        ...
        for v in self.video_infos:
            self.fg_pool.extend([(v.video_id, prop) 
                for prop in v.get_fg(self.train_cfg.ssn.assigner.fg_iou_thr,
                                     self.train_cfg.ssn.sampler.add_gt_as_proposals)])
            incomp, bg = v.get_negatives(self.train_cfg.ssn.assigner.incomplete_iou_thr,
        ...
        denum = self.train_cfg.ssn.sampler.fg_ratio + self.train_cfg.ssn.sampler.bg_ratio + self.train_cfg.ssn.sampler.incomplete_ratio
        self.fg_per_video = int(self.train_cfg.ssn.sampler.num_per_video * (self.train_cfg.ssn.sampler.fg_ratio / denum))
        self.bg_per_video = int(self.train_cfg.ssn.sampler.num_per_video * (self.train_cfg.ssn.sampler.bg_ratio / denum))
        self.incomplete_per_video = self.train_cfg.ssn.sampler.num_per_video - self.fg_per_video - self.bg_per_video
        ...
        if self.verbose:
            ...
            """.format(prop_file=self.proposal_file, pnum=len(self.fg_pool) + len(self.bg_pool) + len(self.incomp_pool),
                       fnum=len(self.fg_pool), inum=len(self.incomp_pool), bnum=len(self.bg_pool),
                       fr=self.fg_per_video, br=self.bg_per_video, ir=self.incomplete_per_video,
                       vnum=len(self.video_infos), vc=self.video_centric, stats=self.reg_stats))
        else:
            print("""
            SSNDataset: proposal file {prop_file} parsed.
    ...
    def _video_centric_sampling(self, record):
        fg = record.get_fg(self.train_cfg.ssn.assigner.fg_iou_thr,
                          self.train_cfg.ssn.sampler.add_gt_as_proposals)
        incomp, bg = record.get_negatives(self.train_cfg.ssn.assigner.incomplete_iou_thr,
                                          self.train_cfg.ssn.assigner.bg_iou_thr,
                                          self.train_cfg.ssn.assigner.bg_coverage_thr,
                                          self.train_cfg.ssn.assigner.incomplete_overlap_thr)
        ...
        out_props = []
        out_props.extend(sample_video_proposals(0, record.video_id, fg, self.fg_per_video, self.fg_pool))
        out_props.extend(sample_video_proposals(1, record.video_id, incomp, self.incomplete_per_video, self.incomp_pool))
        out_props.extend(sample_video_proposals(2, record.video_id, bg, self.bg_per_video, self.bg_pool))
    ...
    def _sample_indices(self, valid_length, num_seg):
        ...
        if average_duration > 0:
            offsets = np.multiply(list(range(num_seg)), average_duration) + np.random.randint(average_duration, size=num_seg)
    ...
    def _sample_ssn_indices(self, prop, frame_cnt):
        ...
        valid_starting = max(1, start_frame - int(duration * self.aug_ratio[0]))
        valid_ending = min(frame_cnt - self.old_length + 1, end_frame + int(duration * self.aug_ratio[1]))

        valid_starting_length = start_frame - valid_starting - self.old_length + 1
        valid_ending_length = valid_ending - end_frame - self.old_length + 1

        starting_scale = (valid_starting_length + self.old_length + 1) / (duration * self.aug_ratio[0])
        ending_scale = (valid_ending_length + self.old_length + 1) / (duration * self.aug_ratio[1])

        starting_offsets = self._sample_indices(valid_starting_length, self.aug_seg[0]) if self.random_shift \
        ...
        ending_offsets = self._sample_indices(valid_ending_length, self.aug_seg[1]) if self.random_shift \
        ...
        stage_split = [self.aug_seg[0], self.aug_seg[0] + self.body_seg, self.aug_seg[0] + self.body_seg + self.aug_seg[1]]
    ...
    def prepare_train_imgs(self, idx):
        ...
        for idx, prop in enumerate(props):
            ...
            for idx, seg_ind in enumerate(prop_indices):
                for i, x in enumerate(range(0, self.old_length, self.new_step)):
                    img_group.extend(self._load_image(osp.join(self.img_prefix, prop[0][0]), image_tmpl, modality, min(frame_cnt, int(seg_ind) + x + skip_offsets[i])))
            ...
            for i, (modality, image_tmpl) in enumerate(zip(self.modalities[1:], self.image_tmpls[1:])):
                ...
                for idx, seg_ind in enumerate(prop_indices):
                    for x in range(0, self.old_length, self.new_step):
                        img_group.extend(self._load_image(osp.join(self.img_prefix, prop[0][0]), image_tmpl, modality, int(seg_ind) + x + skip_offsets[i]))
                        
                flip = True if np.random.rand() < self.flip_ratio else False
                img_group, img_shape, pad_shape, scale_factor, crop_quadruple = self.img_group_transform(img_group, self.img_scale,
                    crop_history=img_meta['crop_quadruple'],
                    flip=dataimg_meta['flip'], keep_ratio=self.resize_keep_ratio,
                    div_255=self.div_255, is_flow=True if modality=='Flow' else False)
                # [L x C x H x W]
                if self.input_format == "NCTHW":
                    img_group = np.transpose(img_group, (1,0,2,3))
            ...
            if prop[1] == 0:
                ...
                reg_targets = (reg_targets[0] - self.reg_stats[0][0]) / self.reg_stats[1][0], \
                              (reg_targets[1] - self.reg_stats[0][1]) / self.reg_stats[1][1]
            else:
                reg_targets = (0., 0.)
    ...
    def prepare_test_imgs(self, idx):
        ...
        frame_ticks = np.arange(0, frame_cnt - self.old_length, self.test_interval, dtype=int) + 1
        ...
        for idx, seg_ind in enumerate(frame_ticks):
            for x in range(0, self.old_length, self.new_step):
                img_group.extend(self._load_image(osp.join(self.img_prefix, video_id), image_tmpl, modality, int(seg_ind) + x))
        ...
        for i, (modality, image_tmpl) in enumerate(zip(self.modalities[1:], self.image_tmpls[1:])):
            img_group = []
            for idx, seg_ind in enumerate(frame_ticks):
                for j, x in enumerate(range(0, self.old_length, self.new_step)):
                    img_group.extend(self._load_image(osp.join(self.img_prefix, video_id), image_tmpl, modality, int(seg_ind) + x + skip_offsets[j]))
