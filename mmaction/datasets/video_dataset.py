...
class VideoDataset(Dataset):
    ...
    def _get_frames(self, record, image_tmpl, modality, indices, skip_offsets):
        if self.use_decord:
            if not modality in ['RGB', 'RGBDiff']:
                raise NotImplementedError
            images = list()
            for seg_ind in indices:
                ...
                for i, ind in enumerate(range(0, self.old_length, self.new_step)):
                    if skip_offsets[i] > 0 and p + skip_offsets[i] <= record.num_frames:
            ...
            for seg_ind in indices:
                ...
                for i, ind in enumerate(range(0, self.old_length, self.new_step)):
                    if p + skip_offsets[i] <= record.num_frames:
                        seg_imgs = self._load_image(osp.join(self.img_prefix, record.path), modality, p + skip_offsets[i]) 
                    else:
                        seg_imgs = self._load_image(osp.join(self.img_prefix, record.path), modality, p) 
                    images.extend(seg_imgs)
                    if p + self.new_step < record.num_frames:
                        p += self.new_step
            return images
    ...
    def __getitem__(self, idx):
        ...
        if self.use_decord:
            self.video_reader = decord.VideoReader('{}.{}'.format(osp.join(self.img_prefix, record.path), self.video_ext))
        else:
            self.video_frames = mmcv.VideoReader('{}.{}'.format(osp.join(self.img_prefix, record.path), self.video_ext))
        if self.test_mode:
