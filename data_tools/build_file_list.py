...
def main():
    ...
    if args.dataset == 'ucf101':
        split_tp = parse_ucf101_splits(key_func=key_func)
    elif args.dataset == 'kinetics400':
        split_tp = parse_kinetics_splits()
    ...
    if len(split_tp) > 1:
        ...
        lists = build_split_list(split_tp[0], frame_info,
                                 shuffle=args.shuffle)
        filename = '{}_train_list_{}.txt'.format(args.dataset,
                                                 args.format)
        with open(osp.join(out_path, filename), 'w') as f:
            f.writelines(lists[0][0])
        filename = '{}_val_list_{}.txt'.format(args.dataset,
                                               args.format)
        with open(osp.join(out_path, filename), 'w') as f:
            f.writelines(lists[0][1])
